...
std::string HelpMessage()
{
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 22011 or testnet: 22012)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 22014 or testnet: 22015)") + "\n";
    ...
    strUsage += "  -loadblock=<file>      " + _("Imports blocks from external zet000?.dat file") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Zetacoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Zetacoin is probably already running."), strDataDir));
