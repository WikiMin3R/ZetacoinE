...
// Copyright (c) 2013-2018 Zetacoin developers
...
// Note: Upon the inception of the "New Zetacoin" source, starting with the Genesis Block and ending with block #175, the code below was set to allow pre-mining of sufficient "New Zetacoins" (1 million coins / block) to facilitate complete 1:1 replacement of existing "Zetacoin-Legacy" coin supply (e.g., 175 million coins). Once block #175 was reached, PoW subsidy was reduced as planned, to 0.1 coins / PoW block. This event took place at around 22:00 PM (New York) Eastern Time.
...
{
    string strBlockFn = strprintf("zet%04u.dat", nFile);
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    {
        try {
            CAutoFile zetdat(fileIn, SER_DISK, CLIENT_VERSION);
            unsigned int nPos = 0;
            while (nPos != (unsigned int)-1 && zetdat.good())
            {
                boost::this_thread::interruption_point();
                unsigned char pchData[65536];
                do {
                    fseek(zetdat, nPos, SEEK_SET);
                    int nRead = fread(pchData, 1, sizeof(pchData), zetdat);
                    if (nRead <= 8)
                    {
                        nPos = (unsigned int)-1;
                        break;
